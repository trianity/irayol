                            ...
                            <div class="card-header container-fluid">
                                ...
                                <form action="{{route('menu.active')}}" method="POST">
                                    ...
                                    <div class="btn-group btn-group-sm float-right" >
                                        <button type="submit" class="btn btn-{{ $indmenu->id == setting('main_menu') ? 'success' : 'secondary' }}">{{ $indmenu->id == setting('main_menu') ? __('global.active') : __('global.inactive') }}</button>
                                        <a href="{{ route('menu.index') }}" class="btn btn-primary" title="Show All Menus"><i class="fa fa-undo" aria-hidden="true"></i> {{__('global.return_back')}}</a>
                                    </div>
                                </form>                                    
                            </div>
                                                ...
                                                <div id="collapseTwo" class="collapse" aria-labelledby="headingOne" data-parent="#accordion01">
                                                    <div class="card-body">
                                                        <div class="mt-3" id="customlinkdiv">
                                                            <input id="custom-menu-item-url" name="url" type="text" class="form-control form-control-sm" placeholder="{{__('global.url')}}" title="url" />
                                                            <input id="custom-menu-item-name" name="label" type="text" class="form-control form-control-sm mt-3" placeholder="{{__('global.title')}}" title="Label menu" />
                                                            <a href="#" onclick="addcustommenu()" class="btn btn-success btn-sm float-right mt-3 mb-3">{{__('global.add')}}</a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                                            ...
                                                            <div class="form-group">
                                                                <div class="input-group mb-3">
                                                                    ...
                                                                    <div class="input-group-append">
                                                                        <a href="#" onclick="addpagemenu(this, {{$page->id}})" class="btn btn-success btn-sm">{{__('global.add')}}</a>
                                                                    </div>
                                                                </div>
                                                            </div>                                                                                                                                    
                                                        @endforeach 
                                                            ...
                                                            <div id="post-body-content">
                                                                @if (count($menus) < 1)
                                                                    <div class="alert alert-dismissible alert-info">
                                                                        ...
                                                                        <h4 class="alert-heading">{{__('global.menu_structure')}}</h4>
                                                                        <p class="mb-0">{{__('global.menu_indication')}}</p>												
                                                                    </div>
                                                                @endif
                                                                    ...
                                                                    <li id="menu-item-{{$m->id}}" class="list-group-item menu-item-handle menu-item menu-item-depth-{{$m->depth}} menu-item-page menu-item-edit-inactive pending" style="display: list-item;">
                                                                        <dt class="menu-item-handle">
                                                                            <span class="item-title">
                                                                                ...
                                                                                <span class="is-submenu" style="@if ($m->depth==0) display: none ; @endif;">{{__('global.subelement')}}</span>
                                                                            </span>
                                                                        ...
                                                                        <div class="menu-item-settings container bg-light text-dark mt-3" id="menu-item-settings-{{$m->id}}">
																			<input type="hidden" class="edit-menu-item-id" name="menuid_{{$m->id}}" value="{{$m->id}}" />
                                                                              <div class="row">
                                                                                <div class="col">
                                                                                    <label class="col-form-label col-form-label-sm" for="edit-menu-item-title-{{$m->id}}">{{__('global.title')}}</label>
                                                                                ...
                                                                                <div class="col">
                                                                                    <label class="col-form-label col-form-label-sm" for="edit-menu-item-classes-{{$m->id}}">{{__('global.class_CSS') .' '. '(' . __('global.optional') . ')' }}</label>
                                                                            ...
                                                                            <label class="col-form-label col-form-label-sm" for="edit-menu-item-url-{{$m->id}}"> {{__('global.url')}} </label>
																			...
																			<span>Move:</span> 
																			<a href="{{ $currentUrl }}" class="menus-move-up" style="display: none;">{{__('global.move_up')}}</a>
                                                                            <a href="{{ $currentUrl }}" class="menus-move-down" title="Mover uno abajo" style="display: inline;">{{__('global.move_down')}}</a>
                                                                            ...
                                                                            <a href="{{ $currentUrl }}" class="menus-move-top" style="display: none;">{{__('global.top')}}</a>

                                                                            <div class="mt-3 mb-3">
                                                                                <a class="btn btn-danger btn-sm item-delete submitdelete deletion" id="delete-{{$m->id}}" href="{{ $currentUrl }}?action=delete-menu-item&menu-item={{$m->id}}&_wpnonce=2844002501">{{__('global.delete')}}</a>
                                                                                <span class="meta-sep hide-if-no-js"> | </span>
                                                                                <a class="btn btn-secondary btn-sm item-cancel hide-if-no-js" id="cancel-{{$m->id}}" href="{{ $currentUrl }}?edit-menu-item={{$m->id}}&cancel=1424297719#menu-item-settings-{{$m->id}}">{{__('global.cancel')}}</a>
                                                                                <span class="meta-sep hide-if-no-js"> | </span>
                                                                                <a onclick="getmenus()" class="btn btn-primary btn-sm updatemenu" id="update-{{$m->id}}" href="javascript:void(0)">{{__('global.update')}}</a>
                                                                            </div>
                                                                        </div>
                                                                    </li>
																	@endforeach 
